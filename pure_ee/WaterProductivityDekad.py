...
def GEE_calc(agb_pass,aet_pass):
    ...
    L1_AGBPSeasonalMasked = agb_pass.map(lambda lista: lista.updateMask(lista.gte(0)))
    ...
    ETaColl1 = aet_pass.map(lambda immagine: immagine.updateMask(immagine.lt(254) and (immagine.gt(0))))                                             
...
def selectRastersInput():
    ...
    data_start = datetime.datetime(2015, 01, 01)
    data_end = datetime.datetime(2015, 02, 01)
